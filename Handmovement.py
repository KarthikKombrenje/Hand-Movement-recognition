...
screenWidth, screenHeight = pyautogui.size()  # Outputs the high and width of the screen (1920 x 1080)
...
while True:
    ...
    if len(lmList) != 0:
        ...
        if finger[1] == 1 and finger[2] == 0:  # Checks to see if the pointing finger is up and thumb finger is down
            x3 = numpy.interp(x1, (75, 640 - 75), (0, screenWidth))  # Converts the width of the window relative to the screen width
            y3 = numpy.interp(y1, (75, 480 - 75), (0, screenHeight))  # Converts the height of the window relative to the screen height
            ...
            pyautogui.moveTo(screenWidth-cX,screenHeight-cY)# Function to move the mouse to the x3 and y3 values (wSrc inverts the direction)
